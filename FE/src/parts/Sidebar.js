...
function Sidebar({ match, history }) {
  ...
  // function logout() {
  //   users.logout().then(() => {
  //     localStorage.removeItem("RAPMICRO:token");
  //     history.push("/login");
  //   });
  // }
  ...
  const sidebarStyle = {
    width: 280,
    left: window.innerWidth < 500 && !toggleMenu ? "-280px" : 0,
  };
      ...
      >
        ...
        >
          <div className="flex flex-col text-center mt-8">
            ...
            <h6 className="text-white text-xl">{USERS?.name ?? "Made By Raka"}</h6>
            <span className="text-indigo-500 text-sm">
              {USERS?.profession ?? "Devops Adira"}
            </span>
          </div>
